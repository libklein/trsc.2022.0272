...
from .legacy_network import VehicleGraph, build_vehicle_graph, render_graph, display_graph, Edge, TourVertex, Vertex, \
    ChargerVertex
...
class DynamicTourModel:
    ...
    def __init__(self, instance: DiscretizedInstance, **cli_args):
        ...
        self.vehicle_networks: Dict[Vehicle, VehicleGraph] = {k: build_vehicle_graph(vehicle=k, model=self.model,
                                                                                     periods=self.periods,
                                                                                     tours=self.tours[k],
                                                                                     battery=self.battery,
                                                                                     breakpoints_per_charger=self.breakpoints_per_charger,
                                                                                     charging_opportunities=self._generate_vehicle_charging_opportunities(k)) for k in self.vehicles}
    ...
    def render_solution(self, vehicle: int = 0) -> dot.Digraph:
        return render_graph(self.vehicle_networks[vehicle], solution=True)

    def display_solution(self, *args, **kwargs):
        self.render_solution(*args, **kwargs).render(view=True)

    def _generate_vehicle_charging_opportunities(self, vehicle: Vehicle):
        periods_covered_by_tours = set()

        for pi in self.tours[vehicle]:
            if pi.latest_departure <= pi.earliest_arrival:
                next_p = pi.latest_departure
                while next_p is not pi.earliest_arrival.succ:
                    periods_covered_by_tours.add(next_p)
                    next_p = next_p.succ

        latest_tour = max(self.tours[vehicle], key=lambda pi: pi.latest_departure)
        return {
            (p, f) for f in self.chargers for p in self.periods
            if p < latest_tour.latest_departure and p not in periods_covered_by_tours
        }
    ...
    def _create_objective(self):
        ...
        if not self._cli_args.get('ignore_degradation'):
            all_tour_vertices: Iterable[TourVertex] = chain(*map(attrgetter('tour_vertices'),
        ...
        if not self._cli_args.get('ignore_energy_price'):
            all_charger_vertices: Iterable[ChargerVertex] = chain(*map(attrgetter('charger_vertices'),
                                                                       self.vehicle_networks.values()))
            energy_cost = self.model.sum(v.gamma * v.period.energyPrice for v in all_charger_vertices)
            self.model.add_kpi(energy_cost, publish_name='Energy Price')
            objective += energy_cost
        else:
            print('Ignoring energy price')

        if not self._cli_args.get('ignore_route_costs'):
            all_vertices: Iterable[Vertex] = chain(*map(attrgetter('vertices'), self.vehicle_networks.values()))
            fix_cost = self.model.sum(v.fix_cost for v in all_vertices)
            self.model.add_kpi(fix_cost, publish_name='Fix Cost')
            objective += fix_cost
        else:
